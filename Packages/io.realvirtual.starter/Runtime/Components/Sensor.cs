...
namespace realvirtual
{
    ...
    //! 
    //! For detailed documentation see: https://doc.realvirtual.io/components-and-scripts/sensors/sensor
    #endregion
    [HelpURL("https://doc.realvirtual.io/components-and-scripts/sensors/sensor")]
    [ExecuteInEditMode]
    [Icon("Packages/io.realvirtual.starter/Editor/EditorAssets/EditorScriptIcons/Sensor Icon.png")]
    public class
        Sensor : BaseSensor, ISignalInterface, IXRPlaceable
    {
        ...
        [BoxGroup("Settings")] [HideIf("UseRaycast")]
        [Tooltip("Continuously detect objects with Physics.OverlapBox every physics step instead of relying only on trigger events. More robust against fast-moving objects (no tunneling) and detects objects already present, at a small per-step cost.")]
        public bool UseOverlapDetection = false; //!< If true the sensor checks its box volume with Physics.OverlapBox every FixedUpdate (collider mode only). Avoids missed detections for fast-moving objects (tunneling) and reliably detects objects already present in the volume. Adds one OverlapBox query per physics step.
        ...
        private int _overlapLayerMask; // collision-matrix mask for OverlapBox seeding/polling (collider mode)
        private static readonly Collider[] _overlapBuffer = new Collider[64]; // shared non-alloc buffer for OverlapBox polling
        ...
        private void Start()
        {
            ...
            {
                Warning("Sensors which are not using a Raycast need to have a BoxCollider on the same Gameobject as this Sensor script is attached to", this);
            }
            ...
            _overlapLayerMask = ComputeOverlapMask();
            ShowStatus();
        ...
        //! Builds a layer mask from the sensor layer's collision matrix (every layer not ignored vs. the sensor layer).
        private int ComputeOverlapMask()
        {
            int sensorLayer = gameObject.layer;
            int mask = 0;
            for (int l = 0; l < 32; l++)
                if (!Physics.GetIgnoreLayerCollision(sensorLayer, l)) mask |= 1 << l;
            return mask;
        }

        //! Detects objects that are already inside the sensor's trigger volume at simulation start.
        //! PhysX only raises OnTriggerEnter on an overlap transition, so a part already sitting in the
        //! sensor when the simulation starts would otherwise go undetected until it moves. Respects the
        //! sensor layer's collision matrix and the LimitSensorToTag filter (via OnTriggerEnter).
        private void SeedInitialOverlaps()
        {
            if (_boxcollider == null) return;

            var t = _boxcollider.transform;
            Vector3 worldCenter = t.TransformPoint(_boxcollider.center);
            Vector3 halfExtents = Vector3.Scale(_boxcollider.size * 0.5f, t.lossyScale);
            var hits = Physics.OverlapBox(worldCenter, halfExtents, t.rotation, _overlapLayerMask, QueryTriggerInteraction.Collide);
            for (int i = 0; i < hits.Length; i++)
            {
                var col = hits[i];
                if (col == null || col == _boxcollider) continue;
                if (col.transform.IsChildOf(transform)) continue; // ignore the sensor's own colliders
                OnTriggerEnter(col);
            }
        }

        //! Polls the sensor's box volume every physics step (collider mode, when UseOverlapDetection is on).
        //! Detects objects that entered or left without an OnTriggerEnter/Exit transition (e.g. fast-moving
        //! objects that would tunnel through). Runs additively next to the trigger callbacks; the Contains
        //! guards in OnTriggerEnter/OnTriggerExit make double processing a no-op.
        private void PollOverlap()
        {
            if (_boxcollider == null) return;
            ...
            Vector3 worldCenter = t.TransformPoint(_boxcollider.center);
            Vector3 halfExtents = Vector3.Scale(_boxcollider.size * 0.5f, t.lossyScale);
            int count = Physics.OverlapBoxNonAlloc(worldCenter, halfExtents, _overlapBuffer, t.rotation,
                _overlapLayerMask, QueryTriggerInteraction.Collide);

            // Enter: any overlapping collider not yet tracked (OnTriggerEnter applies the tag filter and events)
            ...
            // Exit: any tracked object that is no longer inside the volume
            var snapshot = CollidingObjects.ToArray();
            for (int i = 0; i < snapshot.Length; i++)
            {
                ...
                for (int j = 0; j < count; j++)
                {
                    ...
                    if (col != null && col.gameObject == obj) { stillInside = true; break; }
                }
        ...
        private void FixedUpdate()
        {
            ...
            else if (Application.isPlaying && UseOverlapDetection && !_isNetworkControlled)
            {
                PollOverlap();
            }
